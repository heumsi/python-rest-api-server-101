...
def test_handle_successfully(client, headers_with_authorized_admin):
    # when
    response = client.get("/users/", headers=headers_with_authorized_admin)
    ...
    assert links == [{"href": f"{client.base_url}/users/", "rel": "self"}]
...
def test_handle_unsuccessfully_with_no_authorization(
    ...
    # when
    response = client.get("/users/", headers=headers_with_authorized_common)
