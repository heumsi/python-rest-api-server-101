...
def test_handle_successfully(client, common_user, common_another_user):
    ...
    assert data == [
        ...
        },
        {
            ...
            "links": [
                {"href": f"{client.base_url}/v1/comments/2", "rel": "self"},
                {"href": f"{client.base_url}/v1/posts/{post_.id}", "rel": "post"},
            ],
        },
    ]
    links = json_data.get("links")
    assert links == [{"href": f"{client.base_url}/v1/comments/", "rel": "self"}]
...
def test_handle_successfully_with_params_including_post_id(client, common_user):
    ...
    # when
    response = client.get("/v1/comments/", params={"post_id": post_1.id})
...
def test_handle_successfully_with_specific_offset_and_limit(client, common_user):
    ...
    assert links == [
        {
            "href": f"{client.base_url}/v1/comments/?offset=1&limit=1",
            "rel": "self",
        },
        {
            "href": f"{client.base_url}/v1/comments/?offset=2&limit=1",
            "rel": "next",
        },
        {
            "href": f"{client.base_url}/v1/comments/?offset=0&limit=1",
            "rel": "prev",
        },
    ]
