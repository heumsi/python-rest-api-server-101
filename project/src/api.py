...
from src.model import Post, PostBase, PostUpdate
...
def update_post(post_id: int, post_update: PostUpdate) -> Post:
    with Session(engine) as session:
        post = session.get(Post, post_id)
        if not post:
            raise HTTPException(status_code=404, detail="Post not found")
        post.updated_at = int(time.time())
        updated_post_data = post_update.dict(exclude_unset=True)
