...
def handle(
    ...
    with Session(engine) as session:
        ...
        if not post_feedback:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="PostFeedback not found")
        if Role(current_user.role) != Role.ADMIN and post_feedback.user_id != current_user.id:
